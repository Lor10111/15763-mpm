...
{
public:
    constexpr static std::string_view kTestName_ = "translating_cube";

    // material and particle property
    ...
    class MPMTestTranslatingCubeConfig : public MPMConfigBase<MPMTestTranslatingCubeConfig>
    {
    public:
        friend class MPMConfigBase<MPMTestTranslatingCubeConfig>;

        constexpr MPMTestTranslatingCubeConfig() = default;
    ...
    typedef MPMTestTranslatingCubeConfig TestConfig_;
};
...
auto SetupModel(uint32_t& particleCount) -> std::vector<MPMModelVariant>
{
    ...
    Vector<float, 3> initialVelocitySum{0.f, 0.f, 0.f};
    Vector<float, 3> initialMomentum{0.f, 0.f, 0.f};

    auto center = Vector<float, 3>{48, 48, 48} * kDx;
        ...
        for (int j = -4; j <= 4; ++j)
        {
            for (int k = -4; k <= 4; ++k)
            {
                for (int w = 0; w < 8; ++w)
                {
                    ...
                    velocity.emplace_back(Vector<float, 3>{0.05f, 0.0f, 0.0f});

                    initialVelocitySum += velocity.back();
                    initialMomentum += MPMTestScene::kParticleMass_ * velocity.back();
                }
            }
        }
    }
    ...
    printf("Initial momentum: (%f, %f, %f)\n",
           initialMomentum[0], initialMomentum[1], initialMomentum[2]);
