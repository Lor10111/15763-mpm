...
{
public:
    constexpr static std::string_view kTestName_ = "dropping_block_fluid";

    // ── Grid spacing ──────────────────────────────────────────────────────
    constexpr static float kDx_ = 1.0f / 256.f;

    // ── Material: weakly compressible fluid ──────────────────────────────
    ...
    class MPMTestDroppingBlockFluidConfig : public MPMConfigBase<MPMTestDroppingBlockFluidConfig>
    {
    public:
        friend class MPMConfigBase<MPMTestDroppingBlockFluidConfig>;
        constexpr MPMTestDroppingBlockFluidConfig() = default;
    ...
    protected:
        ...
        // Fixed timestep for fluid stability
    ...
    typedef MPMTestDroppingBlockFluidConfig TestConfig_;
};
...
auto SetupModel(uint32_t& particleCount) -> std::vector<MPMModelVariant>
{
    ...
    printf("Fluid particle count: %zu\n", position.size());
