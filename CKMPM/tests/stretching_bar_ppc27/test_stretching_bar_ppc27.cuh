...
{
public:
    constexpr static std::string_view kTestName_ = "stretching_bar_ppc27";
    ...
    // ── 粒子参数（ppc=27, 3×3×3 sub-cell）───────────────────────────────
    constexpr static float kRho_             = 2.f;
    constexpr static float kParticlePerCell_ = 27.0f;
...
auto SetupModel(uint32_t& particleCount) -> std::vector<MPMModelVariant>
{
    ...
    // ── 矩形 bar 粒子生成（3×3×3 sub-cell 采样，ppc=27）─────────────────
    // x: [30,35), y: [30,35), z: [22,42)  →  5×5×20 = 500 格 × 27 = 13500 粒子
    for (int i = 30; i < 35; ++i)
    {
        for (int j = 30; j < 35; ++j)
        {
    ...
    printf("[stretching_bar_ppc27]  particleCount = %u  (5×5×20×27 = 13500)\n", particleCount);
