...
{
public:
    constexpr static std::string_view kTestName_ = "dropping_block_snow";

    // ── Grid spacing ──────────────────────────────────────────────────────
    constexpr static float kDx_ = 1.0f / 256.f;

    // ── Material: Non-Associated Cam-Clay (snow-like) ─────────────────────
    constexpr static auto kConstitutiveModel_ = MPMConstitutiveModel::kNonAssociatedCamClay;
    constexpr static float kE_              = 2e4f;
    ...
    constexpr static float kAlpha0_         = -0.01f;   // initial yield surface size (compressive)
    constexpr static float kBeta_           = 0.5f;     // tension/compression asymmetry
    constexpr static float kHardeningFactor_ = 0.8f;   // isotropic hardening rate
    constexpr static float kM_              = 2.36f;    // slope of critical state line

    // Constructor order: {lambda, mu, alpha0, beta, hardeningFactor, m, hardeningOn}
    constexpr static auto kMaterial_ =
        MPMMaterial<kConstitutiveModel_>{
            MPMMaterial<kConstitutiveModel_>::NonAssociatedCamClayMaterialParameter{
                kLambda_, kMu_, kAlpha0_, kBeta_, kHardeningFactor_, kM_, true
            }
        };

    // ── Particle parameters ───────────────────────────────────────────────
    constexpr static float kRho_             = 500.f;
    ...
    class MPMTestDroppingBlockSnowConfig : public MPMConfigBase<MPMTestDroppingBlockSnowConfig>
    {
    public:
        friend class MPMConfigBase<MPMTestDroppingBlockSnowConfig>;
        constexpr MPMTestDroppingBlockSnowConfig() = default;
    ...
    protected:
        ...
        // Fixed timestep for snow/CamClay stability
    ...
    typedef MPMTestDroppingBlockSnowConfig TestConfig_;
};
