...
namespace test{
    ...
    {
    public:
        constexpr static std::string_view kTestName_ = "droplet";
        ...
        // constexpr static float kTotalSimulatedTime_ = 340.f / 48.f;
        ...
        class MPMTestDropletConfig : public MPMConfigBase<MPMTestDropletConfig>
        {
           public:
            friend class MPMConfigBase<MPMTestDropletConfig>;

            constexpr MPMTestDropletConfig() = default;
        ...
        typedef MPMTestDropletConfig TestConfig_;
    };
    ...
    auto SetupModel(uint32_t& particleCount) -> std::vector<MPMModelVariant>
    {
        ...
        // modelList.push_back(MPMModel<std::decay_t<decltype(MPMTestScene::kMaterial_)>>{MPMTestScene::kParticleMass_, MPMTestScene::kParticleVolume_, position, velocity, MPMTestScene::kMaterial_, 0.0});
            ...
            for(int j = -radius; j < radius; ++j)
            {
                for(int k = -radius; k < radius; ++k)
                {
                    ...
                    for(int w = 0; w < 8; ++w)
                    {
                        ...
                        float particleX = (i + 0.25f + 0.5f * dx + 128) * kDx;
                        float particleY = (j + 0.25f + 0.5f * dy + 128) * kDx;
                        float particleZ = (k + 0.25f + 0.5f * dz + 128) * kDx;

                        auto pos = Vector<float, 3>{particleX, particleY, particleZ};
                        if((pos - center).Norm() < radius * kDx)
                        {
        ...
        // modelList.push_back(MPMModel<std::decay_t<decltype(MPMTestScene::kMaterial_)>>{MPMTestScene::kParticleMass_, MPMTestScene::kParticleVolume_, position, velocity, MPMTestScene::kMaterial_, 5.0});
        modelList.push_back(MPMModel<std::decay_t<decltype(MPMTestScene::kMaterial_)>>{MPMTestScene::kParticleMass_, MPMTestScene::kParticleVolume_, position, velocity, MPMTestScene::kMaterial_, 0.0});
