...
#   Top Dirichlet (+ω): z∈[37,42)    Bot Dirichlet (−ω): z∈[22,27)
#
# Note: CKMPM loads particles from .bin (cylindrical bar).
#       Here we generate a rectangular bar analytically – same physics.
#       This is the quadratic-kernel equivalent of the "yellow bar" in Fig.10.
...
OUT_GIF = os.path.join(OUT_DIR, "twisting_bar_mlsmpm.gif")
os.makedirs(OUT_DIR, exist_ok=True)

print(f"[mlsmpm-twist]  dx={DX:.4e}  c_s={C_S:.4f}  dt={DT:.4e}")
print(f"[mlsmpm-twist]  particle_vol={PARTICLE_VOL:.4e}  particle_mass={PARTICLE_MASS:.4e}")

# ── Bar geometry ──────────────────────────────────────────────────────────────
X_LO, X_HI    = 30, 35
Y_LO, Y_HI    = 30, 35
Z_LO,  Z_HI   = 22, 42
Z_TOP_LO, Z_TOP_HI = 37, 42   # top end Dirichlet zone (+ω)
Z_BOT_LO, Z_BOT_HI = 22, 27   # bot end Dirichlet zone (−ω)
...
def render(frames):
    ...
    plt.close(fig)
    print(f"[mlsmpm-twist]  GIF → {OUT_GIF}")


if __name__ == "__main__":
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"[mlsmpm-twist]  device={device}")
    frames = run(device)
    render(frames)
