...
# Outputs: output/twisting_bar_basic3d.gif
#
...
#   Top end (+ω): z∈[37,42)    Bot end (−ω): z∈[22,27)
...
OUT_GIF = os.path.join(OUT_DIR, "twisting_bar_basic3d.gif")
...
print(f"[basic3d-twist]  dx={dx:.4e}  c_s={c_s:.4f}  dt={dt:.4e}")

# ── Bar geometry ──────────────────────────────────────────────────────────────
X_LO, X_HI      = 30, 35
Y_LO, Y_HI      = 30, 35
Z_LO,  Z_HI     = 22, 42
Z_TOP_LO, Z_TOP_HI = 37, 42
Z_BOT_LO, Z_BOT_HI = 22, 27
...
N = len(all_pos)
print(f"[basic3d-twist]  particles: {N}")
...
print(f"[basic3d-twist]  steps_per_frame={steps_per_frame}  total_frames={total_frames}")
...
print("[basic3d-twist]  simulation done, rendering …")
...
ax.set_title("Twisting Bar – Basic MPM 3D  (quadratic kernel, ppc=8)")
...
plt.close(fig)
print(f"[basic3d-twist]  GIF → {OUT_GIF}")
